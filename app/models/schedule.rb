class Schedule 
  ...
  def self.events(day)
    ...
    when :tuesday
      [conditioning(day, 5.5), open_gym(day, 6.5, 2.5), conditioning(day, 9), personal_training(day, 10, 5), open_gym(day, 15, 2.5), conditioning(day, 17.5), open_gym(day, 17.5)]
    when :wednesday
      [open_gym(day, 5.5, 3.5), empty(day, 9), personal_training(day, 10, 5), open_gym(day, 15, 5)]
    when :thursday
      [conditioning(day, 5.5), open_gym(day, 6.5, 2.5), conditioning(day, 9), personal_training(day, 10, 5), open_gym(day, 15, 2.5), conditioning(day, 17.5), open_gym(day, 17.5)]
  ...
  def self.open_gym(date, start_hour, duration=1)
    event("Open Gym", "Drop in anytime", "open-gym", date, start_hour, duration)
  end

  def self.conditioning(date, start_hour, duration=1)
    event("Conditioning", "Open gym with limited coaching", "conditioning", date, start_hour, duration)
  end

  def self.personal_training(date, start_hour, duration=1)
    event("Personal Training", "Dedicated Personal Training Only", "personal-training", date, start_hour, duration)
  end
