# config valid only for current version of Capistrano
lock "3.14.1"

set :application, "FlightPerformanceAndFitness"
set :repo_url, "git@github.com:eliotpiering/flight-performance-and-fitness.git"


set :rbenv_type, :user # or :system, depends on your rbenv setup
set :rbenv_ruby, '2.6.6'

set :rbenv_prefix, "RBENV_ROOT=#{fetch(:rbenv_path)} RBENV_VERSION=#{fetch(:rbenv_ruby)} #{fetch(:rbenv_path)}/bin/rbenv exec"
set :rbenv_map_bins, %w{rake gem bundle ruby rails puma pumactl}
set :rbenv_roles, :all

set :nvm_node, 'v12.16.1'
set :nvm_type, :system # or :system, depends on your nvm setup
set :nvm_map_bins, %w{node npm yarn}

#https://github.com/koenpunt/capistrano-nvm/issues/25#issuecomment-320806448
set :default_env, {
  "PATH" => "$HOME/.nvm/versions/node/v12.16.1/bin:$PATH"
}

#after 'nvm:validate', 'nvm:webpacker:validate'
#before 'deploy:assets:precompile', 'nvm:webpacker:wrap'
#nvm_wrapper not working with assets:precompileafter 'deploy:assets:precompile', 'nvm:webpacker:unwrap'

# Default branch is :master
# ask :branch, `git rev-parse --abbrev-ref HEAD`.chomp

# Default deploy_to directory is /var/www/my_app_name
# set :deploy_to, "/var/www/my_app_name"

# Default value for :format is :airbrussh.
# set :format, :airbrussh

# You can configure the Airbrussh format using :format_options.
# These are the defaults.
# set :format_options, command_output: true, log_file: "log/capistrano.log", color: :auto, truncate: :auto

# Default value for :pty is false
# set :pty, true

# Default value for :linked_files is []
append :linked_files, "Flight_prod"

# Default value for linked_dirs is []
append :linked_dirs, "log", "tmp/pids", "tmp/cache", "tmp/sockets", "public/system", "storage"

# Default value for default_env is {}
# set :default_env, { path: "/opt/ruby/bin:$PATH" }

# Default value for local_user is ENV['USER']
# set :local_user, -> { `git config user.name`.chomp }

# Default value for keep_releases is 5
# set :keep_releases, 5
